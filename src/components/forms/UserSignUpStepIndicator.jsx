  ...
  if (currStep > stepNo)
    return (
      <div className="flex flex-row justify-center items-center">
        <div className="w-8 h-8 rounded-full border-none bg-gradient-to-r from-blue-500 to-blue-300 border-2 items-center flex justify-center flex-row">
          <TiTick className="text-white text-xl" />
        </div>
        {lastIndicator ? (
          <span></span>
        ) : (
          <span className="md:w-24 w-16 bg-blue-500 h-0.5 mx-3"></span>
        )}
      </div>
    );
  else
